 ...
 */
def executeSuite(suite, config, results, overall)
{
    ...
    targetHosts.split(",").each
    {
        ...
        browsers.split(",").each
        {
            ...
            try
            {
                ...
                //FIXME queue and run from a thread pool instead so that we can
                //      parallelise tests
...
def executeSuiteWithCommandProcessor(suiteFile, cp, results, overall)
{
    ...
    html.body[0].table[0].tbody[0].tr.each
    {
        ...
        if (row.td[0].a)
        {
            ...
            {
                log.warn("Test '$suiteFile' not found. It will be ignored.")
            }
            catch (SAXParseException spx)
            {
                results.pass=false
...
else
{
    suites = ant.fileScanner 
    {
        ...
        {
            include(name: "**/suite-failed.html")
        }
    }
}
...
try
{
    for (s in suites)
    {
        ...
        executeSuite(s, config, results.suites[suiteName], results)
    }
}
catch (Exception x)
{
